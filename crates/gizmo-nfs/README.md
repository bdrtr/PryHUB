...
| `GEOMETRY.BIN` **writing** | `geometry::write` | ✅ done for a mesh at its own topology — `replace_mesh` rewrites a solid's vertices, indices and bounding box, and **all 609 meshes in a real car write back byte-identically**. The bbox is not the vertices' AABB but min−0.01 / max+0.01 (23,292 of 23,299 real solids), which is why writing one back unchanged reproduces the file. Changing the vertex or triangle *count* is refused rather than guessed at: it needs the mesh header's counts, the 60-byte submesh runs and the 128-byte alignment in front of the buffers. Underneath, `rebuild` is the repacker plus `rebuild` is the repacker plus the one thing it cannot know: `0x00134004` is a per-solid table of **absolute file offsets**, one 24-byte record per solid, and growing any chunk strands it. Measured over 18,230 records: lane 1 is the solid's offset, lanes 2 and 3 its size + 8, lanes 4–5 zero — 18,230/18,230 each, and the records are *not* in file order. A real car rebuilt with a grown vertex buffer keeps every record true where the plain repacker strands 100+, and a no-op rebuild is byte-identical. What is still missing above this line is a mesh *importer* |
