...
| World / city (`STREAM*.BUN`) | `world` | ✅ geometry and textures — **28,985 objects and all 5,087 textures** across the eight bundles. `header` reads the `0x00134011` record through the `0x11` filler the city prefixes it with, which is what the car path's absolute `MATRIX_OFFSET` cannot do: skipping it, the matrix's own last element is `1.0` in 175 of 175 L4RH headers, and not skipping it in the 46 that had no filler — a wrong read that yields plausible matrices rather than failing. `manifest` walks a bundle for every object's identity, extent, placement, counts and texture slots without touching a buffer (324 MB in 0.2 s, 134 MB peak); `object` reads the buffers, at the 24-byte stride the city uses — asked for rather than inferred, since `layout_for` would try 36 first. `tpk` reads the city's own pack variant: **no `0x33310003` in any of the 502 packs**, a `0x33310004` table of `0x7C`-byte records instead, and the pixel format **stated** at `+0x4A` rather than inferred from the size. The pool base is the `0x33320002` payload past its 120 bytes of filler, which is what makes the furthest extent in a pack land exactly on the pool's end in 502 of 502; the palettes sit *ahead* of the images, the reverse of a car pack. 69,150 of 70,439 texture slots (98.17 %) resolve in their own bundle — the rest are in `TRACKS/LOC4DYNTEX.BIN` (which is the **car** variant) or under `GLOBAL/`, and 111 resolve nowhere at all. What is left here is the route files and `L4R*.BUN` |
